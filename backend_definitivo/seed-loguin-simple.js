...
async function seedLoguinSimple() {
    try {
        console.log('🔧 Configurando seed para tabla loguin...');
        ...
        // Importar modelos usando la configuración del proyecto
        const dbConfig = require('./src/config/db').default;
        const Loguin = dbConfig.models.find(m => m.name === 'Loguin');
        const Empleados = dbConfig.models.find(m => m.name === 'Empleados');
        const Roles = dbConfig.models.find(m => m.name === 'Roles');
